...
class Game:
  ...
  def run(self):
    ...
    while running:
      ...
      goto(-175,62)
      write("add : "+str(self.ajout)+" | cost : "+str(self.price_ajout)+" | key : 1")
      goto(-175,49)
      ...
      self.all_achi,self.achi,self.achi_nb,self.achi_1,self.achi_1000,self.achi_50000,self.achi_ajout_10,self.achi_auto_10,self.achi_speed_5,self.achi_speed_20,self.achi_coquin=achievement_fonc(
        self.cheat,self.cookie,self.ajout,self.auto,self.auto_speed,self.all_achi,self.achi,self.achi_nb,self.achi_1,self.achi_1000,self.achi_50000,self.achi_ajout_10,self.achi_auto_10,self.achi_speed_5,self.achi_speed_20,self.achi_coquin)
      key=getkey()
      if key==K_OK :
        self.cookie+=self.ajout
        clear()
      elif key==K_1:
        if self.cookie>=self.price_ajout:
          self.ajout+=1  
          self.cookie-=self.price_ajout
          self.price_ajout=int(self.price_ajout*1.7)
          clear()
      elif key==K_2:
        ...
        running=end(self.cookie,self.ajout,self.auto,self.auto_speed,self.achi,self.achi_nb)
      wait(9990)
      compteur+=1
      if running:
        if compteur==self.auto_speed:
          compteur=0
